...
To investigate this cross-phenomenon transfer boundary, four detector architectures spanning three design philosophies are evaluated. The models are trained on smoke annotations alone and tested zero-shot against an external fire dataset.
...
These findings demonstrate a clear architectural dependency in cross-domain transfer. Convolutional networks (YOLO11n and Faster R-CNN) rely heavily on local kernel patterns and edge textures; when presented with flame colors and sharp luminous gradients instead of diffuse smoke, their activation maps fail to trigger valid proposals. In contrast, transformer architectures with deformable attention learn spatial query relationships that generalize across physically related phenomena, detecting fire anomalies in nearly 60\% of unseen test images under relaxed spatial overlap constraints. This research was supported by the College of Artificial Intelligence at the Arab Academy for Science, Technology and Maritime Transport.

\section{Conclusion}
This study investigated whether object detectors trained strictly on smoke imagery can detect wildland fires zero-shot. While standard convolutional models exhibit near-total transfer failure under strict bounding-box matching, transformer architectures retain meaningful cross-phenomenon sensitivity, detecting flames in up to 58.8\% of test images under operational early-warning spatial thresholds. These results suggest that transformer-based vision systems trained on abundant daytime smoke plumes can provide functional anomaly detection for active fires in aerial forest surveillance.
