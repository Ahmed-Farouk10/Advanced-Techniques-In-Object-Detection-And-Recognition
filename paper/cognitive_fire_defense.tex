...
\caption{Distribution of images, brightness, and box area across the three splits produced by the clip-level constraint optimizer. Each split contains at least one clip from each of the four geographic locations. Small plumes are present in all splits.}
\label{fig:split}
\end{figure}
...
\begin{equation}
\min_{\mathcal{S}} \sum_{s \in \{\text{train},\text{val},\text{test}\}} \left(\frac{N_s}{N} - r_s\right)^2 + \lambda_1 \sigma_{\text{blur}} + \lambda_2 \mathbb{I}[\text{small}_s = 0]
\label{eq:split_opt}
\end{equation}
...
\begin{table}[H]
\centering
\caption{Final split distribution. The 63/19/17 ratio deviates from the 70/15/15 target because 31 indivisible blocks cannot be partitioned into three buckets with arbitrary precision while satisfying all four constraints simultaneously.}
\label{tab:split}
...
The limitations of the current results must be stated plainly. The Faster R-CNN model was trained for only five epochs, far short of convergence, and its transfer performance may improve with a full training schedule. The YOLO11n training was conducted on CPU over approximately twelve hours, and while this does not affect the validity of the within-domain baseline, it precluded the multi-seed statistical validation that would quantify the variance of the transfer results. Finally, the split used for these experiments (80/20 train/validation) differs from the 70/15/15 clip-level split described in the methodology; both are temporally leakage-free, but the numbers are not directly comparable across the two protocols.
